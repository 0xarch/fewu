...
function generateSitemapXml(root_url,posts){
    ...
    result +=`<url><loc>${root_url}</loc><priority>0.9</priority></url>`;
    for(let v of posts){
        result+=`<url><loc>${root_url+v.websitePath}</loc><lastmod>${v.datz.toString()}</lastmod><priority>1.0</priority></url>`;
    }
    result += '</urlset>';
    return result;
}
